...
CHECKPOINT   : CP-033
TIMESTAMP    : 2026-08-28T01:55:00+05:30
TRIGGER      : task-complete
SESSION      : 6 (continued)
PHASE        : P9 in progress
BATCH        : B16 in progress (T-P9-01 through T-P9-06 done; T-P9-07, T-P9-08 next, G9)
COMPLETED    : T-P9-06 (Documentation reconciled against as-built)
EVIDENCE     : Audited every file under `docs/` against actual `src/` exports and implementations. Confirmed via `src/index.ts` → `pipeline`/`components`/`theme` barrels that `<Markdown>`, `<StreamingMarkdown>`, `useMarkdown`, `renderToReact`, `configureCache`/`clearCache`, a configurable `PipelineOptions`, and `<ThemeProvider tokens/fonts/defaultTheme/storageKey>` were never implemented — none of those names exist anywhere in `src/`. Rewrote `API.md` in full against the real surface: `<MarkdownRoot>`, `<ThemeProvider>` (children-only), `<ThemeToggle>` (no props), `useStreamingMarkdown`, `useTheme` (`{theme, setTheme}`, no `'system'`/`resolvedTheme`), `processor` (fixed, non-configurable), `toReact`/`SubtreeCache`, `isFastPathEligible`/`fastPathRender`, `safeUrl`, streaming internals. Independently found `src/components/Lightbox.tsx` is never imported by `Image.tsx` (confirmed via grep) — clicking an image does not open a lightbox, contradicting `SPEC.md` FR-5.1, `USER-GUIDE.md`, and `CHANGELOG.md`'s planned "Added" list; annotated all three with as-built notes rather than deleting the aspirational content outright (SPEC.md is the requirements record; CHANGELOG.md's `[1.0.0] — planned` section is explicit forward-looking scope for T-P9-07 to finalize). Corrected `ARCHITECTURE.md`'s module map and `INSTALLATION.md`'s minimal-integration example, Next.js snippet, and configuration-options table. Cross-checked the 34-language syntax-highlighting claim against `src/pipeline/plugins/shiki-config.ts` — exact match, no correction needed. `SECURITY.md` was already reconciled at T-P8-09/CP-026, not touched again here.
PENDING      : T-P9-07 (release notes + `1.0.0` version across all manifests — must resolve the CHANGELOG.md gaps flagged this checkpoint: cache/lightbox/token-override/font-override items in the planned "Added" list do not reflect the as-built surface); T-P9-08 (delivery checklist + human acceptance request); GATE G9.
VALIDATION   : PASS — every `docs/*.md` file now either accurately describes the as-built surface or carries an explicit "As-built note" flagging where it still describes a planned-but-unimplemented feature, with the gap named specifically enough for T-P9-07 to act on.
ISSUES       : none open
ASSUMPTIONS  : Left `SPEC.md`'s FR-5.1 and `CHANGELOG.md`'s planned "Added" list textually intact (annotated rather than rewritten) on the assumption that requirements-record and pre-release-target documents should show what was planned alongside what shipped, rather than being silently edited to match current reality — if this assumption is wrong, human can request T-P9-07 rewrite these outright instead of annotating.
DEFERRED     : DEF-001; KaTeX lazy-loading boundary gap; the two pre-existing lint errors flagged at CP-016; DEC-019's S-01 flakiness; Lightbox's unwired production integration (now documented in `docs/` rather than only in the ledger — still unresolved in code; fixing it is a product decision, not this task's scope); the mermaid.spec.ts test-ordering flake; the 8 remaining devDependency-only pnpm-audit findings; APK signing/release-keystore setup (out of scope for any P9 task as currently scoped). The `docs/API.md` vs. as-built gap that had been carried forward since T-P9-01 is now RESOLVED (this checkpoint) — removed from the deferred list.
NEXT TASK    : T-P9-07 (release notes and `1.0.0` version bump across `package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`, `src-tauri/gen/android/app/build.gradle.kts`, and any other version-bearing manifest; finalize `docs/CHANGELOG.md`'s `[1.0.0]` section against the as-built gaps flagged at this checkpoint)
CONTEXT USED : not tracked precisely this session
DECISIONS    : Annotated rather than deleted the aspirational/planned content in `SPEC.md` and `CHANGELOG.md` (Lightbox, cache config, token/font overrides) — those documents' purpose (requirements record; pre-release changelog target) differs from `API.md`/`ARCHITECTURE.md`/`INSTALLATION.md`'s purpose (describe the callable surface as it exists today), so the reconciliation strategy differs by document rather than being uniform.
CORRECTIONS  : none this checkpoint.
DERIVATIONS  : none new this checkpoint.
─────────────────────────────────────────────
...
─────────────────────────────────────────────
CHECKPOINT   : CP-036
TIMESTAMP    : 2026-08-28T03:20:00+05:30
TRIGGER      : human-testing-found-gap
SESSION      : 6 (continued)
PHASE        : P9 complete, re-verification before re-requesting G9
BATCH        : B16 complete
COMPLETED    : Fixed a real product gap CP-035's G9 request had not caught: `src/app/main.tsx` — the single entry point both `pnpm build:app` (PWA) and the Tauri desktop shell build against — mounted only `<ThemeProvider><ThemeToggle /></ThemeProvider>`, a leftover T-P7-08 test harness never built out into the actual "read rendered documents" reader `SPEC.md §2`/`§5` promises for the PWA/desktop deliverables. Found by the human running the app themselves per `docs/HUMAN-TESTING-GUIDE.md` and reporting "its doing nothing it's just a button to swtich between light mode and dark mose" — CP-035's "PWA build exists and installs"/"Tauri binary exists and launches" checklist items had been satisfied purely on the artifacts *building*, never on whether the built app fulfills its stated end-user use case. The human explicitly challenged the initial framing of this as needing "a demo page" ("why isnt the product working??"), correctly identifying it as a real completeness gap, not cosmetic — plan approved at `/home/vipul/.claude/plans/fluttering-imagining-tiger.md`, with the added standing directive to keep pursuing full product completion, not just this one fix.
EVIDENCE     : Rewrote `src/app/main.tsx` into a `Reader` component using only existing exported API (`useStreamingMarkdown`, `MarkdownRoot`, `ThemeProvider`, `ThemeToggle` — zero new dependencies, zero other files touched): seeds a hand-written sample Markdown string revealed via a timer-driven `useStreamingMarkdown` call (demonstrating streaming on load), plus a toggleable `<textarea>` for pasting/typing custom Markdown with live re-render. `pnpm tsc` → 0 errors. Browser-verified via Chrome tooling against `npx vite --mode app --port 5173`: real content renders (heading/list/code/table), caught and fixed a React console warning (`cloneElement(element, {key: index})` for the `elements` array, justified by FR-3.3 monotonicity — block position is stable once emitted). While verifying the theme toggle with real content on top (not possible to notice with the old toggle-only harness, since near-invisible text on a near-invisible background looked the same as "nothing rendered"), found and fixed a second, independent, pre-existing bug in `src/theme/ThemeProvider.tsx` itself: `index.html`'s anti-FOUC script sets `data-theme` on `<html>` once at load (the attribute `tokens.css`'s `--surface`/`--text-primary` key off), but `ThemeProvider` only ever set `data-theme` on its own inner `.claymark-root` div — toggling theme post-mount (or a live OS-scheme change) desynced the two, leaving text color flipped to the new theme's palette while the background token stayed stuck on whichever theme was active at load, producing near-invisible low-contrast text. Confirmed via `getComputedStyle` (`rootColor: rgb(245,245,244)` i.e. near-white text over `bodyBg: rgb(250,250,250)` near-white background while `data-theme=dark` was active) and via screenshot. Fixed with a `useEffect` in `ThemeProvider` syncing `document.documentElement.dataset.theme` on every `theme` change. Re-ran `pnpm tsc` → 0 errors. Re-verified in-browser: dark mode now shows correct dark background/white text, theme choice persists correctly across a full page reload with no FOUC, live textarea editing re-renders correctly in both themes, zero console errors/warnings. Ran the full regression suite: `pnpm test` → 118/118 passing (including the previously-flaky S-01 stress scenario, DEF-005 — passed cleanly this run, not treated as newly-fixed since it's a known timing-margin flake, not a determinstic pass). `pnpm build:app` → builds cleanly (`dist/app/index.html` 1.87 kB, `main-*.css` 6.26 kB, `sw.js` 0.72 kB, `main-*.js` 311.68 kB gzip 97.56 kB); served the built output (`npx serve dist/app`) and re-confirmed identical correct rendering in the browser.
PENDING      : GATE G9 — human acceptance, re-requested after this fix (see message to human accompanying this checkpoint).
VALIDATION   : PASS — the PWA/desktop entry point now genuinely fulfills `SPEC.md §2`'s "read and interact with rendered documents" end-user use case, verified end-to-end (dev server, built-and-served output, both themes, live custom input, full regression suite), rather than resting on "the artifact builds" as CP-035 had.
ISSUES       : The Tauri desktop binary itself was not rebuilt/re-launched this checkpoint (no interactive display available in this headless session to visually confirm the native window) — the reader-entry-point fix is verified via the PWA build (same `src/app/main.tsx`/`index.html`), not via a native Tauri re-launch. Flagged as residual risk in `plan/03-CHECKLIST.md`, not silently assumed equivalent.
ASSUMPTIONS  : Treated the PWA dev-server + built-and-served browser verification as sufficient evidence that the Tauri desktop shell (which loads the identical `index.html`/`src/app/main.tsx` bundle via its own webview) also renders correctly, since Tauri's webview is a standard Chromium/WebKit engine loading the same static assets — no Tauri-specific rendering divergence is expected, but this was not independently confirmed by launching the actual desktop binary.
DEFERRED     : No new items — DEF-001..DEF-008 stand unchanged. This checkpoint's two fixes (Reader entry point, ThemeProvider dark-mode contrast bug) are corrections to already-flagged gaps, not newly deferred work.
NEXT TASK    : none — awaiting human acceptance at GATE G9, this time with the end-user reading use case actually verified rather than assumed from a successful build.
CONTEXT USED : not tracked precisely this session
DECISIONS    : Fixed the `ThemeProvider` contrast bug at its source (`src/theme/ThemeProvider.tsx`) rather than working around it in `main.tsx`, since the bug affects every consumer of the library's theme system, not just this one entry point.
CORRECTIONS  : `plan/03-CHECKLIST.md`'s "PWA build exists and installs"/"Tauri binary exists and launches" VERIFY notes, corrected to reflect that the end-user use case is now actually verified, not just that the build compiles. `docs/HUMAN-TESTING-GUIDE.md`'s "Fastest way to see it running" section, re-verified/updated against the real Reader UI's actual appearance and behavior.
DERIVATIONS  : none new this checkpoint.
─────────────────────────────────────────────
