...
| D-010 | Bumped vite/vitest/@vitejs-plugin-react/playwright/typescript across major/minor versions to close DEF-007 (L-025), rather than leaving the critical/high findings open | DEF-007's own ticket described only "8 devDependency-only" findings needing a lighter touch; actual `pnpm audit` showed 15, including a critical (vitest) and highs requiring real toolchain version bumps, plus one production-reaching finding. Surfaced this gap via `AskUserQuestion` with three options (safe-overrides-only / do-everything / overrides-and-stop); user chose to do everything, explicitly accepting the breakage risk the assistant had flagged |
...
| L-023 | Missing `dist/styles.css` (surfaced during DEF-003, fixed on explicit user request as a follow-up) | `vite.config.ts`'s lib build had no CSS entry point at all — `docs/INSTALLATION.md`'s documented `import 'claymark/styles.css'` resolved to nothing in a consumer's build, silently (no error) | Added `src/styles.css` (`@import`s `theme/tokens.css`, `theme/claymark.css`, `theme/katex.css` in that order). Added it as a second `build.lib.entry` in `vite.config.ts` (Vite's CSS plugin accepts a `.css` file as a lib entry directly) and set `cssCodeSplit: true` (Vite defaults it `false` in lib mode, which refuses a CSS rollup input outright). Added `"./styles.css": "./dist/styles.css"` to `package.json`'s `exports`. Changed `sideEffects: false` → `sideEffects: ["*.css"]` — a bare `false` would let an aggressive bundler legally tree-shake away a side-effect-only `import 'claymark/styles.css'` in consumer code, since nothing consumes that module's exports | `pnpm tsc --noEmit` 0 errors; `pnpm lint` same 3 pre-existing errors (DEF-004), 0 new; `pnpm build` exit 0 — `dist/claymark.js`/`.cjs` unchanged (807B/1043B, code-splitting intact), `dist/styles.css` now exists (37KB; contains real KaTeX classes, `.claymark-button` rules, `@font-face` declarations); `pnpm build:app` exit 0. Full suite: 122/123 — the one failure (`tests/useStreamingMarkdown.spec.tsx`, an async-hydration timing assertion) passes 4/4 in isolation, confirming a pre-existing full-suite-load flake unrelated to this change. Commit `6445358` | PASS |
| L-024 | DEF-004 — 3 pre-existing lint errors | `pnpm lint` findings, traced each to its actual cause rather than suppressing | Two (`react/no-danger`, `react-hooks/exhaustive-deps` reported as "Definition for rule not found") were a config gap: `eslint-plugin-react`/`eslint-plugin-react-hooks` were never installed despite two justified `eslint-disable` comments in the codebase referencing their rules. Installed both, exact-pinned (`7.37.5`/`7.1.1`); `eslint-plugin-react` needs `eslint ^9.7`, so bumped `eslint` `9.3.0` → `9.7.0` (exact-pinned, minimal bump clearing the peer-dependency warning). Wired both plugins into `eslint.config.js` with just the two referenced rules. Re-linting then surfaced a real, previously-masked bug: `MermaidDiagram.tsx`'s `eslint-disable-next-line react/no-danger` sat before a two-line explanatory comment rather than directly above `dangerouslySetInnerHTML`, so it suppressed nothing on the actual violating line (ESLint reported both the real violation and an "unused directive" warning on the old line) — moved the directive to immediately precede the prop. Third finding was genuine: `tests/useStreamingMarkdown.spec.tsx`'s `lengths` array is only ever `.push`ed, never reassigned — `let` → `const` | `pnpm lint` exit 0, 0 errors (was 3); `pnpm tsc --noEmit` 0 errors; `pnpm build` exit 0, `dist/claymark.js`/`.cjs` unchanged. Full suite: 122/123 — the one failure (`tests/stress.spec.ts` S-01, a full-suite-load timing flake in files untouched by this change) passes 2/2 in isolation, confirming the pre-existing DEF-005 flake pattern, not a regression. Commit `a819158` | PASS |
| L-025 | DEF-007 — `pnpm audit` findings | `pnpm audit`, cross-checked with `pnpm audit --prod` to test the ticket's "devDependency-only" claim | Actual count was 15 (1 critical, 7 high, 7 moderate), not the ticket's 8, and one (moderate, uuid via mermaid) reaches production, not devDependencies-only — a real gap between the ticket and reality, surfaced via `AskUserQuestion` rather than silently reconciled; user chose to fix everything including the toolchain major bumps. Transitive-only findings (minimatch, ajv, lodash, js-yaml, uuid) fixed via `pnpm.overrides`, each pinned to its first patched version — no top-level version change. `esbuild` override pinned to an exact `0.25.9` rather than left open (`>=0.25.0`): the open range resolved to `0.28.2`, which broke `pnpm build` outright (a genuine esbuild regression transforming destructuring for `mermaid.core`'s target list, reproduced directly). Direct devDependency bumps, each peer-compatibility-checked via registry metadata before applying: `vite` 5.4.21→6.4.3 (required `@vitejs/plugin-react` 4.3.4→5.2.0, and `typescript` 5.4.5→5.6.3 — plugin-react 5.2.0's `.d.ts` uses an arbitrary-module-namespace export syntax TS didn't parse until 5.6, confirmed via a minimal repro against 5.4/5.5/5.6 directly); `vitest` 1.6.1→4.1.11 via 3.2.6 first (vitest 4's peer needed vite already on 6.x); `@vitest/coverage-v8` kept in lockstep (peer pins the exact vitest version); `vite-plugin-dts` 5.0.3→5.1.0; `playwright` 1.44.0→1.61.0, not the audit's minimum 1.55.1 — 1.55.1/1.59.1 both failed `playwright install chromium` on this sandbox's Ubuntu 26.04 ("Playwright does not support chromium on ubuntu26.04-x64", a real install-time gate, confirmed by probing 1.55–1.63 directly); 1.61.0 is the first version whose bundled Chromium revision installs here | `pnpm audit` → "No known vulnerabilities found" (was 15). `pnpm tsc --noEmit` 0 errors. `pnpm lint` 0 errors. `pnpm build` exit 0, `dist/claymark.js`/`.cjs` unchanged, `dist/styles.css` intact. `pnpm build:app` exit 0. `tests/responsive.spec.ts` 3/3 (needed the new playwright's browser binary). Full suite: 121–122/123 across repeated runs — the only failures are `tests/stress.spec.ts` S-01, the already-documented DEF-005 full-suite-load flake, confirmed passing 2/2 in isolation each time. Commit `02d11cf` | PASS |
